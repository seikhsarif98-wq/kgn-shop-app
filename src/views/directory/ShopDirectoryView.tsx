  ...
  return (
    <div id="shop-directory-root" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {activeTab === 'directory' ? (
                  ...
                  <div className="p-5 space-y-4 flex-1 flex flex-col justify-between text-xs">
                    ...
                    <div className="pt-3 border-t border-slate-100 flex items-center gap-2">
                      <button
                        ...
                        className="flex-1 py-2 px-3 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-xs transition flex items-center justify-center gap-1.5"
                      >
                      ...
                      <button
                        id={`manage-shop-${shop.id}`}
                        onClick={() => {
                          switchDemoRole('shop_owner', shop.shopOwnerId);
                          onSelectShop(shop.id, 'admin');
                        }}
                        className="py-2 px-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 font-semibold text-xs rounded-xl transition flex items-center gap-1.5"
                        title="Manage Storefront & POS"
                      >
                        <span>Manage</span>
        ...
        /* PLATFORM OVERVIEW / SUPER ADMIN TAB */
        <div className="space-y-6 animate-in fade-in">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-hidden">
            <div className="p-5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-slate-900 text-sm">Tenant Partition Database Table</h3>
              ...
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50 text-[11px] font-bold uppercase text-slate-500 tracking-wider">
                    ...
                    <th className="py-3 px-4">Tenant Owner ID</th>
                    <th className="py-3 px-4">City</th>
                    <th className="py-3 px-4">Tier Plan</th>
                    <th className="py-3 px-4">Status</th>
                    <th className="py-3 px-4 text-right">Switch Tenant</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {shops.map((s) => (
                    <tr key={s.id} className="hover:bg-slate-50/80 transition">
                      ...
                      <td className="py-3 px-4 font-mono text-[11px] text-blue-700">{s.shopOwnerId}</td>
                      ...
                      <td className="py-3 px-4 text-right">
                        <button
                          onClick={() => {
                            switchDemoRole('shop_owner', s.shopOwnerId);
                            onSelectShop(s.id, 'admin');
                          }}
